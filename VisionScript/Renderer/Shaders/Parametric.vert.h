...
"layout (location = 2) out struct Segment"
"{"
"	vec2 center;"
"	vec2 size;"
"	vec2 dir;"
...
"	if (gl_VertexIndex % 6 == 0) { pos += (forward + right) * radius; }"
"	if (gl_VertexIndex % 6 == 1) { pos += (forward - right) * radius; }"
"	if (gl_VertexIndex % 6 == 2) { pos += (forward + right) * radius; segment.dir *= -1; }"
"	if (gl_VertexIndex % 6 == 3) { pos += (forward + right) * radius; segment.dir *= -1; }"
"	if (gl_VertexIndex % 6 == 4) { pos += (forward - right) * radius; segment.dir *= -1; }"
"	if (gl_VertexIndex % 6 == 5) { pos += (forward + right) * radius; }"
"	vertexColor = color;"
"	fragPosition = pos;"
